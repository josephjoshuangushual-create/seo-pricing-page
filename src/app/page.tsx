...
/**
 * SEO Pricing Landing Page (React + Tailwind)
 * – Full page with all sections restored
 * – Growth card gets green "Best Value" badge
 * – Final CTA "See Packages" anchors to #packages (same page)
 */
...
function useTypewriter(
  phrases: string[],
  opts: { type?: number; erase?: number; pause?: number; loop?: boolean } = {}
) {
  ...
  return { text, index, phase } as const;
}
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            setActiveStep(idx);
            setDotPos((entry.target as HTMLElement).offsetTop + (entry.target as HTMLElement).clientHeight / 2);
          }
        });
      },
      { threshold: 0.6 }
      ...
      <Section id="journey" className="bg-slate-900 text-white">
        <Container>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {[
              {
                icon: <Search className="h-10 w-10 mx-auto text-blue-400" />,
                title: "Months 1–2",
                text: "Audit & foundation. Don’t expect big jumps yet.",
              },
              {
                icon: <PenTool className="h-10 w-10 mx-auto text-green-400" />,
                title: "Months 3–4",
                text: "Early wins: keywords start moving, traffic stabilizes.",
              },
              {
                icon: <TrendingUp className="h-10 w-10 mx-auto text-yellow-400" />,
                title: "Months 5–6",
                text: "Growth momentum. Rankings lift, leads start flowing.",
              },
      ...
      <Section id="costs" className="bg-slate-50 dark:bg-slate-800">
        <Container className="grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <p className="text-slate-700 dark:text-slate-300">
              Our packages are built in line with U.S. benchmarks (Ahrefs, Backlinko) — competitive, transparent, and
              ROI-focused.
            </p>
          </div>
      ...
      <Section id="roi" className="bg-slate-900 text-white">
        <Container>
          ...
          <div className="text-center">
            <Button className={btnSecondary} onClick={() => setShowROI(true)}>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
              <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 w-full max-w-md">
                ...
                <div className="flex justify-end gap-3">
                  <Button className={btnSecondary} onClick={() => setShowROI(false)}>
                    Close
                  </Button>
                </div>
              </div>
          ...
          <div className="relative mx-auto max-w-4xl" ref={containerRef}>
            ...
            {steps.map((step, i) => (
              <div
                key={i}
                data-index={i}
                className={`mb-24 flex w-full items-center justify-between ${i % 2 === 0 ? "flex-row" : "flex-row-reverse"}`}
              >
            ...
            {[
              {
                ...
                role: "DTC Skincare",
                quote:
                  "Within 6 months, organic revenue became our #1 channel. The quarterly ROI reviews keep us focused on what works.",
              },
              {
                name: "Marcus T.",
                role: "B2B SaaS",
                quote:
                  "Technical cleanup + content clusters unlocked rankings we chased for a year. Consistent leads every week now.",
              },
              {
                name: "Lena K.",
                role: "Multi-location Clinic",
                quote:
                  "Local pages + reviews strategy lifted calls across all locations. Transparent, methodical, effective.",
              },
            ].map((t, i) => (
              <motion.div
                key={i}
      ...
      <Section id="cta" className="relative text-white">
        ...
        <Container className="text-center">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
            <Button size="lg" className={btnPrimary} asChild>
              <a href="https://calendly.com/josephjoshua/discovery-call" target="_blank" rel="noreferrer">
                Book My Free SEO Call
              </a>
            </Button>
            <Button size="lg" variant="outline" className={btnSecondary} asChild>
              <a href="#packages" onClick={(e) => { e.preventDefault(); goTo('packages'); }}>See Packages</a>
            </Button>
          </div>
        </Container>
